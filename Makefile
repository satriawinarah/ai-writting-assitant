...
# BACKEND SETUP
#------------------------------------------------------------------------------
setup-backend: $(VENV)/bin/activate build-backend migrate
...
Restart=always
RestartSec=10
StandardOutput=append:$(BACKEND_DIR)/app.log
StandardError=append:$(BACKEND_DIR)/error.log
...
full-setup: install-deps setup-backend build setup-systemd setup-nginx setup-firewall
	...
	@echo "3. (Optional) Run 'make setup-ssl' to enable HTTPS"
	@echo ""
